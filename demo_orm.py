...
class Table(Base):
    ...
    def insert_into(self, list_values):
        if len(list_values) != len(self.name_col)-1:
            print('invalid args', len(list_values), len(self.name_col))
 #           print('invalid args')
            return 1
...
class Select(Base):
    ...
    def sel_all_with_jn(self, obj_tab_fk, obj_tab):
        ...
        sql_str = sqlib.sql_join(obj_tab_fk.name_t, obj_tab.name_t, list_col, \
            obj_tab_fk.fkid[:-1], obj_tab_fk.fkey)
        self.cursor.execute(sql_str)
        self.commit()
        rows =  self.cursor.fetchall()
        list_respns = [row for row in rows]
...
if __name__ == '__main__':
    ...
    type_data = ['INTEGER PRIMARY KEY AUTOINCREMENT','TEXT', 'TEXT', 'TEXT', 'INTEGER']
